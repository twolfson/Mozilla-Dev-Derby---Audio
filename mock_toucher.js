(function () {
  ...
  hasTouch = hasTouch || !!('createTouch' in document);
  hasTouch = hasTouch || !!(typeof TouchEvent != "undefined");
  hasTouch = hasTouch || !!(typeof Touch == "object");
  hasTouch = hasTouch || !!('ontouchend' in document);

  if (!hasTouch) {
    // Create a document-wide MockToucher
    var mocker = new MockToucher(document),
        collection = mocker.collection;

    // Conditionally show circles
    document.addEventListener('mousedown', function () {
      if (collection.pseudoClicks.length === 0) {
        collection.hideCircles();
      } else {
        collection.showCircles();
      }
    });
